...
![*Preliminary PCA to check structure*](PCApriorclustering.png)
...
This is a cumulative density function (CDF) plot of the consensus matrices for the test data from 2...maxK. In the perfect case there would be a flat line to indicate a matrix of just 0s and 1s. CDF flatness can be quantified using the PAC metric (Senbabaoglu et al., 2014). However, in the CDF and following PAC plot we can see the inherant bias of consensus clustering where as K increases so does the apparent stability of the results (or CDF plot flatness), this we correct for by using a reference. This makes the method more sensitive to detection of the underlying structure in noisy data.
\newline  
  
![*CDF plot for real data*](CDF.png)
...
![*PAC score for real data*](PACscore.png){width=90%}
...
![*Empirical p values*](pscore.png){width=90%}
...
![*aheatmap of GBM consensus clusters with tumour classification *](aheatmapGBMfour-vin2.png)
...
![*aheatmap of GBM consensus matrix *](aheatmapconsensusmatrix-vin2.png)
...
![*PCA of a 4 cluster test dataset *](PCApostclustering.png)

![*t-SNE of a 4 cluster simulated dataset *](TSNEpostclustering.png)
...
![*PCA of a 4 cluster simulated dataset *](PCAsim.png){width=80%}
