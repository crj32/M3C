...
#'
#' This is a flexible t-SNE function that can be run on a standard data frame (or the M3C results object).
...
tsne <- function(mydata, K=FALSE, labels=FALSE, perplex=15, printres=FALSE, seed=FALSE, axistextsize = 18,
  ...
  if (K == FALSE && labels == FALSE && text == FALSE){
    ...
    if (printres == TRUE){
      message('printing t-SNE to current directory...')
      png('TSNEpostM3C.png', height = printheight, width = printwidth, units = 'cm',
          res = 900, type = 'cairo')
      print(p) # print ggplot CDF in main plotting window
      dev.off()
    }
