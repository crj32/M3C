...
A feature filter function has been included in M3C that uses the coefficient of variation (CV) to filter features of a data frame. To use we set the CV percentile we wish to filter by and input the data. This code extracts the 10% most variable features.

```{r}
  filtered_data <- featurefilter(mydata, 90)
```
