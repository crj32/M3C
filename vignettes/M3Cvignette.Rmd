...
Genome-wide data is used to stratify large complex datasets into classes using class discovery algorithms. The Monti consensus clustering algorithm is a widely applied method to identify classes, however; it is prone to overfitting and false positives. As a solution, we developed Monte Carlo reference-based consensus clustering (M3C). M3C uses a multi-core enabled Monte Carlo simulation to generate null distributions along the range of K which are used to select its value. Using a reference, that maintains the correlation structure of the input features, eliminates the limitations of the Monti consensus clustering method. M3C uses the Relative Cluster Stability Index (RCSI) and p values to decide on the value of K and reject the null hypothesis, K=1. 
...
The second method (method=2) that uses a penalty term to eliminate overfitting is faster, but you will not get p values for each value of K.
