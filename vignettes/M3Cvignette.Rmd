...
It is also important to check batch effects and other unwanted sources of variation do not drive the main variation in the data. The PCA function has a labels flag for adding categorical and continuous variables to the below plot (type ?pca), but for a more quantitative analysis of these variables before clustering there is David Watson's plot_drivers excellent function from the bioplotr package (https://github.com/dswatson/bioplotr).
