...
Here we can see the PCSI is minimised at K=5, this is the optimal K using this method.

```{r,fig.width=4,fig.height=3}
res$plots[[3]]
```

For this method, it is faster than running a Monte Carlo simulation so we increase the 'repsreal' parameter to e.g. 250 for increased result stability.
...
This is the PCSI calculated from entropy, now K=5, so the bias has been eliminated.
